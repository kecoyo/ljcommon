...
const Navbar: FC<INavbarProps> = ({ location, darkPrefix }) => {
  ...
  return (
    <div className={classNames('__dumi-default-navbar', isHomePage && homeStyle.root)}>
      ...
      <div className='right-part'>
        <nav>
          {navItems.map(nav => {
            ...
            const span = <span key={key}>{nav.path ? <NavLink to={nav.path}>{nav.title}</NavLink> : <a>{nav.title}</a>}</span>;
            return actions ? (
